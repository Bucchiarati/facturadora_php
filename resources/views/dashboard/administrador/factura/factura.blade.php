...
@section('content')
    ...
    <div class="row m-0 p-0 mt-1">
        <div class="col-md-3 px-1"> 
            <h4 class="bg-dark text-center p-1 text-white"> Nota de Entrega Nro: {{$codigo_factura}} </h4>
            <div class="card">
                ...
                <div class="card-body p-3 text-center">
                    <a href="{{route('dashboard.abortar',['id'=>$factura->id, 'modo'=>'ADMIN'])}}" class="card-link">
                        <i class="material-icons align-middle"> arrow_back </i>
                        <span class="align-middle  py-2"> Cancelar </span>
                    </a>
                    <a href="{{route('dashboard.consolidar',['modo'=>'ADMIN'])}}" class="card-link">
                    ...
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item">
                                ...
                                @if(session()->get('detalles'))
                                    @foreach(session()->get('detalles'); as $detalle)
                                        <tr class="text-center">
                                            ...
                                            <td> {{number_format(($detalle->producto->precio_venta + $detalle->iva), 2, '.', ',')}}</td>
                                            <td> {{$detalle->cantidad}}</td>
                                            <td> {{number_format((($detalle->producto->precio_venta + $detalle->iva) * $detalle->cantidad), 2, '.', ',')}}</td>
                                            <td class="p-0 py-1"> 
                            ...
                            @if (session()->get('detalles'))
                                ...
                                <a class="btn btn-secondary" href="{{route('pdf.imprimirnota')}}" role="button" target="_blank">Imprimir Factura <i class="material-icons right">local_printshop</i></a>
                            @endif
                        </li>
                    </ul>
                </div>
